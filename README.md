...
- “3.16 扳手错质检”支持在 QGIS 3.28 中通过独立 QGIS 3.16 子进程运行；使用该功能的电脑仍需安装 QGIS 3.16（64 位）运行环境。插件包已内置 shpchecker 编译扩展，不需要另行安装 shpchecker。默认输入目录为插件目录下的 `转换数据/output`，3.28 不加载这 19 个图层，数据仅在 3.16 中加载。工具栏图标单击运行，双击可更换目录；结果自动导出到输入目录的 `errorlog.xlsx`。
