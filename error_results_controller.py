...
class ErrorResultsDialog(QDialog):
    def __init__(self, controller, parent=None):
        ...
        self.close_button.clicked.connect(lambda *args: self.close())
    ...
    def _build_rules_page(self):
        ...
        invert_button.clicked.connect(lambda *args: self._invert_rule_selection())
        ...
        self.run_quality_button.clicked.connect(lambda *args: self._load_quality_errors())
        ...
        self.run_wrench_button.clicked.connect(lambda *args: self._load_shpchecker_errors())
        buttons.addWidget(self.run_wrench_button)
        self.run_rules_button = QPushButton("执行选中规则", page)
        self.run_rules_button.clicked.connect(lambda *args: self._run_selected_rules())
    ...
    def _build_results_page(self):
        ...
        self.show_rules_button.clicked.connect(lambda *args: self.show_rules())
        buttons.addWidget(self.show_rules_button)
        clear_button = QPushButton("清空记录", page)
        clear_button.clicked.connect(lambda *args: self._clear_results())
        buttons.addWidget(clear_button)
        fix_button = QPushButton("修复选中错误", page)
        fix_button.setToolTip("支持 Shift/Ctrl 多选；双击单条记录也可修复")
        fix_button.clicked.connect(lambda *args: self._fix_selected_records())
